...
def create_filename(cfg, input=False):
    if input:
        filename = cfg['output_pattern'].format(
            step_name=cfg['previous_step_name'])
    else:
        filename = cfg['output_pattern'].format(step_name=cfg['step_name'])
    filename = filename.format(**cfg)
